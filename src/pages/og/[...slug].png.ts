...
async function fetchNotoSansSCFonts() {
  ...
  try {
    // 优先从本地字体文件读取
    const regularBuffer = fs.readFileSync('./public/fonts/NotoSansSC-Regular.ttf');
    const boldBuffer = fs.readFileSync('./public/fonts/NotoSansSC-Bold.ttf');
    
    fontCache = { regular: regularBuffer, bold: boldBuffer };
    return fontCache;
  } catch (err) {
    console.warn("Error reading local fonts, falling back to Google Fonts:", err);
    // 后备：尝试从 Google Fonts 获取
    try {
      ...
      if (!regularUrl || !boldUrl) {
        throw new Error("Could not find font urls in Google Fonts CSS");
      }
      ...
      if (!rResp.ok || !bResp.ok) {
        throw new Error("Failed to download font files from Google");
      }
